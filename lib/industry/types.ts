...
export type ProjectStatusEvent = {date:string;status:DataCenterProject['status'];sourceUrl:string;capacityMw?:number;note?:string};
export type DataCenterProject = {id:string;name:string;owner:string;operator?:string;country:string;region?:string;city?:string;stateCode?:string;status:'announced'|'planning'|'approval'|'construction'|'partially_operational'|'operational'|'delayed'|'cancelled'|'unknown';announcedAt?:string;expectedConstructionAt?:string;expectedOperationalAt?:string;investmentValue?:number;investmentCurrency?:string;powerCapacityMw?:number;sourceUrls:string[];statusHistory?:ProjectStatusEvent[];lastVerifiedAt:string;isEstimated:boolean;notes?:string};
