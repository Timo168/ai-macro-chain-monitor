...
六个中文视图、12 组图表、18 个真实历史序列。使用 React 19 / Vinext / Recharts，支持电脑和手机。不使用演示行情、投资评分或未经核验的市场预期。
...
JS 验证包括 12 组图表各 3 点（36 点）、15 个 FRED 序列各 3 个原始点、辅助通胀转换及缺值/均值/变化单位。Python 验证世界银行各 3 点、修订留存、失败保留缓存与日历日期。报告在 `docs/verification.json`，来源核验在 `docs/source-audit.md`。
...
5. 第二阶段：云厂商资本开支、数据中心建设、半导体订单与盈利，以及天然气、区域电价、铝。
