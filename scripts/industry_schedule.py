...
def run(force=False):
    ...
    if force or current-last>=timedelta(hours=24) or state.get('collectorVersion')!=6:
        failures=[]
        ...
        atomic(path,{'collectorVersion':6,'lastAttemptAt':now(),'failures':failures,'nextCheckAt':(current+timedelta(hours=24)).isoformat()})
